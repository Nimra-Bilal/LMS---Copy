...
const CourseDetails = () => {
    ...
    const {allCourses , calcAvgRating , currency , calcNoOfLect , calcCourseDuration , calcCourseChapTime } = useContext (AppContext)
...
<h1 className='font-semi-bold text-2xl md:text-4xl text-gray-800'>{courseData.courseTitle}</h1>
<p className='md:text-base text-sm pt-4 ' dangerouslySetInnerHTML={{__html:courseData.courseDescription.slice(0,200)}}></p>
{/* review n rating */}
<div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
                <p>{calcAvgRating(courseData)}</p>
                <div className='flex'>
                    {[...Array(5)].map((_,i)=>(<img key={i} 
     src={i < Math.floor(calcAvgRating(courseData)) ? assets.star : assets.star_blank} 
     alt="" 
     className='w-3.5 h-3.5' />
          ...
          <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'students' : 'student'}</p>
            ...
            <p className='text-sm'>Course by <span className='text-blue-600 underline'>Nimra</span></p>
       
       <div className='pt-8 text-gray-800'>
<h2 className='text-xl font-semibold'>Course Structure</h2>
<div className="pt-5">{courseData.courseContent.map((chapter,index)=>(
    <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
    <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={()=>toggleFunct(index)}>
...
<div className='py-20 text-sm md:text-default rich-text'>
...
{/* right col */}
<div className='z-10 max-w-[424px] shadow-[0px_4px_15px_2px_rgba(0,0,0,0.1)] rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px] '>
{
...
<div className='h-4 w-px bg-gray-500/40'></div>
    ...
    <div className='flex items-center gap-1'>
        ...
        <p>{calcNoOfLect(courseData)} Lessons</p>
    </div>
