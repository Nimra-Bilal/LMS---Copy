...
const Player = () => {
  const {enrolledCourses , calcCourseChapTime 
    , backendUrl , getToken , userData , fetchUserEnrolledCourses
...
if(enrolledCourses.length>0){
  ...
  const markLectAsCompleted = async (lectureId)=>{
try {
  const Token = await getToken()
  const { data } = await axios.post((backendUrl+ '/api/user/update-course-progress'), 
  {lectureId , courseId} , {headers:{Authorization: `Bearer ${Token}`}})
...
getCourseProgress()
  ...
  return courseData ?  (
    <>
    <div className='p-4 sm:p-10 flex flex-col-reverse md:grid md:grid-cols-2 gap-10 px-36 '>
      {/* left col  */}
  ...
  <div> 
    <YouTube videoId={playerData.lectureUrl.split('/').pop()} 
           ...
           <div className='flex justify-between items-center mt-1'>
            ...
            <button onClick={()=>markLectAsCompleted(playerData.lectureId)} className='text-blue-600'>
