...
<div className='grid grid-cols-[repeat(auto-fit,_minmax(200px,_1fr))] px-4 md:px-0 md:my-16 my-10 gap-4
 '>
  {allCourses.slice(0,4).map((course,index)=><CourseCard key={index} course={course} />)}
      ...
      <Link to={'/course-list'} onClick={()=>scrollTo(0,0)} className="text-gray-500 border border-gray-500/30 px-10 py-3 rounded transition duration-300 ease-in-out transform hover:scale-205 hover:text-yellow-100 hover:bg-blue-500 hover:shadow-xl " >Show all courses</Link>
    </div>
  )
}
